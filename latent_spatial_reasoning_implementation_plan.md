...
**Stretch result.** Emitted latent thoughts, when decoded to nearest real views, recover a human-interpretable viewpoint trajectory that matches the teacher agent's trace ≥60% of the time (trace-IoU metric defined in §8).
...
│   ├── data/                      #   dataset + teacher-agent configs
...
│   ├── data/
│   │   ├── teacher_agent/         # 3D tool-calling trace generator
│   │   ├── trace_filters.py       # coherence + validity filters
...
### 4.2 Teacher agent (the step-level trace generator)

This is the single highest-risk piece; scope it first.

Implementation: a tool-calling loop driven by **Qwen2.5-VL-72B** (or GPT-4o as backup oracle). Tools:

1. `reconstruct(scene_id)` → VGGT features, BEV grid.
2. `detect(view, classes)` → boxes.
3. `segment(view, box)` → mask.
4. `orient(view, mask)` → canonical yaw.
5. `measure(view_a, view_b, mask_a, mask_b)` → metric distance via depth lift.
6. `rotate_mental(scene_id, pose_delta)` → new rendered view.
7. `bev_sketch(scene_id, objects[])` → top-down plot with object markers.
8. `python(code)` → sandboxed exec for arithmetic / comparisons.

Each tool call emits `(view, camera_pose)` — this is the **step-level ground-truth latent target**.

Loop template:

```
while not answerable(state):
    action = agent.step(query, state)       # LLM picks a tool + args
    view, pose, tool_out = run_tool(action)
    state.append(view=view, pose=pose, tool_out=tool_out)
trace = {query, steps: state, final_answer}
```

### 4.3 Filters

- **Trace coherence.** For step `i>1`, require cosine-sim between `SigLIP(view_i)` and `SigLIP(view_{i-1})` ∈ [0.3, 0.92]. Too low → agent jumped incoherently; too high → degenerate near-duplicate step.
- **Trace validity.** The teacher's final answer must match the dataset ground truth. For open-ended answers, use GPT-4o judge with a strict rubric; keep only unambiguous agreements.
- **Length filter.** Keep traces with 2–6 tool-call steps. <2 is trivial; >6 dilutes supervision.

Expected yield: 30–40% of attempted queries produce a kept trace. Target **~120k kept traces** for Stage 2 — enough based on 3DThinker's sample efficiency.
...
Where each `L_i` is one token slot whose label is the frozen target
...
- Data: 120k teacher-filtered traces + 30k general VQA (replay to prevent catastrophic forgetting).
- Losses:
...
- Data: same traces, but during forward pass the latent-token embeddings are the model's *own* projected latent head outputs, not the teacher's targets. (This is the transition from teacher-forcing to free generation; use scheduled sampling — 80% teacher → 0% over 1 epoch.)
...
- **Nearest-view decoding.** For each emitted latent, retrieve the scene view with highest cosine sim under the frozen SigLIP. Report top-1 accuracy against the teacher's trace view (chance ≈ 1/#views).
- **Trace-IoU.** Fraction of emitted latents whose nearest real view shares ≥50% of pixels (via re-projected depth) with the teacher's ground-truth step view.
- **Pose error.** Angular + translational error between predicted pose and teacher pose, when the model is made to emit an explicit pose token (pose-regression variant).
...
- **Target pose = from teacher trace**, not sampled from a prior. Reason: the whole point is to ground to *useful* viewpoints, and the teacher's tool use already picks those.
...
- **Latent-step budget = 4** on average at training time (teacher traces filtered to 2–6). Ablate 1/2/4/6 at inference.
...
| Teacher trace generation | 5 | 4×A100 (for VLM-72B judge) | 480 |
...
| **Total** | **~29 days** | | **~4.3k GPU-hours** |

Fits comfortably on an 8×A100 node running 4 weeks, or scale out to 3 nodes for ~2 weeks.
...
|---|---|---|
| Teacher agent yield < 30% | Medium | Fall back to a smaller template-based trace synthesizer for "distance", "counting", "size" — these are rule-expressible without an LLM agent. |
...
1. **M1 (week 1):** Teacher agent yields ≥30% kept traces over 5k queries. *Gate: if <15%, pause and template-synthesize.*
2. **M2 (week 2):** Stage 1 projectors trained; MMMU regression ≤1 pt.
3. **M3 (week 4):** Stage 2 teacher-forced latent cosine-sim ≥0.8; SQA3D val ≥ base VLM + 2 pts.
...
2. Write the teacher agent over 3 tools (`reconstruct`, `detect`, `measure`) end-to-end on ScanQA. Goal: 100 traces, eyeball-inspect 20, iterate on filters.
3. Pre-train the pose encoder (§3) — small, should land in half a day.
4. Scale teacher agent to full toolbox + parallelize to 4 GPUs. Target 120k traces by end of week 2.
5. Implement trace packer + collate and land a unit test that round-trips a packed sample to loss without exceptions.
6. Kick off Stage 1 on 4×A100 while trace generation runs in parallel on the remaining GPUs.
